...
### Opencode Runner Requests Default Output
...
```
Given the run_tests_opencode.py file
Then it passes "--format default" to opencode CLI
Because structured verdict extraction requires predictable output format
```
...
### Both Runners Enforce Execution Timeout
...
```
Given run_tests_claude.py and run_tests_opencode.py
Then both enforce a timeout on CLI execution (at least 60 seconds, at most 300)
Because hung tests must not block CI pipelines indefinitely
```

### Both Runners Report Errors Without Crashing
...
```
Given run_tests_claude.py and run_tests_opencode.py
Then both return a TestResult with error information when CLI fails
...
### Both Runners Load External Prompt Template
...
```
Given run_tests_claude.py and run_tests_opencode.py
Then both load the judge prompt from judge_prompt.md in the scripts directory
Because separating prompt from code enables easier iteration and review
```

### Both Runners Substitute Test Variables
...
```
Given run_tests_claude.py and run_tests_opencode.py
Then both substitute these placeholders when rendering the prompt:
  - {{target_name}}
  - {{target_content}}
  - {{test_name}}
  - {{test_section}}
